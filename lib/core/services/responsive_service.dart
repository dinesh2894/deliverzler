...
class ResponsiveService {
  ...
  /// Use MediaQueryData.formWindow(window) instead of MediaQuery.of(context)
  ...
  static double availableScreenHeight(BuildContext context, {double ratio = 1}) =>
      (MediaQuery.of(context).size.height - MediaQueryData.fromView(window).viewPadding.vertical) *
      ratio;

  static double availableScreenWidth(BuildContext context, {double ratio = 1}) =>
      (MediaQuery.of(context).size.width - MediaQueryData.fromView(window).viewPadding.horizontal) *
      ratio;
  ...
  // Notes: This doesn't get Android's bottom system bar as flutter already ignoring it when getting screen size
