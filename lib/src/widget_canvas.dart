...
class WidgetCanvasRenderTwoDimensionalViewport<T> extends RenderTwoDimensionalViewport {
  ...
  List<CanvasElement<T>> getVisibleElement(
      WidgetCanvasElements<T> elements, Offset topLeft, Offset bottomRight) {
  ...
  void layoutChildSequence() {
    ...
    final bottomRight = Offset(horizontalPixels, verticalPixels);
    ...
    if (canvasDelegate.showGrid) {
      ...
      for (var column = 0; column < viewportWidth ~/ dimension; column += 1) {
        if (buildOrObtainChildFor(ChildVicinity(xIndex: column, yIndex: rulerVerticalLayer))
            case final child?) {
          child.layout(constraints.loosen());
          parentDataOf(child).layoutOffset =
              Offset((baseColumn + column) * dimension, 0) - Offset(horizontalPixels, 0);
        }
      }

      final baseRow = (verticalPixels / dimension).floor();
      for (var row = 0; row < viewportHeight ~/ dimension; row += 1) {
        if (buildOrObtainChildFor(ChildVicinity(xIndex: row, yIndex: rulerHorizontalLayer))
            case final child?) {
          child.layout(constraints.loosen());
          parentDataOf(child).layoutOffset =
              Offset(0, (baseRow + row) * dimension) - Offset(0, verticalPixels);
        }
      }
    }
...
class WidgetCanvasChildDelegate<T> extends TwoDimensionalChildDelegate {
  ...
  Widget? build(BuildContext context, ChildVicinity vicinity) {
    ...
    if (_sortedElements[vicinity] case final elements?) {
      return builder(context, elements);
    }
